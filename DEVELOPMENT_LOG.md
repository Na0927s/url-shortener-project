...
- **개발 기간:** 2일
- **목표:** Gemini CLI와 GitHub를 활용하여 프로젝트를 체계적으로 만들고 나타내 보자.
- **작업 내용**: 긴 URL을 입력하면 고유한 단축 코드를 생성하여 제공, URL 리디렉션, 실시간 유효성 검사, 클립보그 복사, 히스토리
...
### ### 19 - 클립보드 복사 기능 추가
- **작업 내용:** 단축된 URL을 쉽게 복사할 수 있도록 `public/index.html`에 '복사' 버튼을 추가하고, `navigator.clipboard.writeText()`를 사용하여 클립보드에 복사하는 기능을 구현함.
- **AI 활용:** Gemini CLI의 `replace` 도구를 사용하여 `index.html` 파일에 버튼과 스크립트를 추가함.

### ### 20 - DEVELOPMENT_LOG.md 충돌 발생 해결
- **작업 내용:** 각 로컬 main 브랜치와 원격 origin/main 브랜치 최신 커밋(DEVELOPMENT_LOG.md)들로 인한 충돌 발생.
- **AI 활용:** Gemini를 통해 해결 방범을 찾음. git reset --soft HEAD~1 명령어로 바로 직전 커밋으로 되돌림. git reset --hard 0d88501명령어로 해당 커밋 삭제, 이후 삭제된 로컬사항을 다시 추가 하며 문제 해결함.

### ### 21 - URL 히스토리 기능 추가 및 목록 개수 제한
- **작업 내용:** URL 단축기 애플리케이션에 히스토리 기능을 추가함. 여기에는 히스토리 목록 표시, 각 항목 옆에 삭제 버튼 추가, 로컬 스토리지에서 URL 저장 및 삭제 기능 구현이 포함됨. 또한, 히스토리 목록을 10개로 제한하고, 10개를 초과하면 가장 오래된 항목을 삭제하도록 구현함.
- **AI 활용:** Gemini CLI의 `replace` 도구를 사용하여 `index.html` 파일에 히스토리 관련 HTML 요소와 JavaScript 로직을 추가하고, 히스토리 길이 제한 로직을 추가함.

### ### 22 - README.md 기술 스택 추가
- **작업 내용:** `README.md`에 개발 스택을 추가함

### ### 23 - DEVELOPMENT_LOG.md 3차 수정 작성
- **작업 내용:** `DEVELOPMENT_LOG.md`에 클립보드 복사 기능 추가, 충돌 발생 해결, URL 히스토리 기능 추가 및 목록 개수 제한 추가로 넣어 3차 완성함.
...
- **URL 입력줄 삭제:** URL을 입력하고 새로운 URL을 압력해야 하는 과정에서 지우는 번거로움이 있다. 삭제 버튼을 추가하거나, URL을 선택하면 전체적으로 선택되어 쉽게 지울 수 있도록 바꿀 계획이다.
